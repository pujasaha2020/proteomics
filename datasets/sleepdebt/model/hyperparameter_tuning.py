...
def extract_top_coefficients(cv_results_zscore, data_training):
    ...
    for i, model in enumerate(cv_results_zscore["estimator"]):
        ...
        pipeline = model.best_estimator_.model_
        lr_model = pipeline.named_steps["elasticnet"]
...
def hyperparameters_tuning(
    ...
    resampled_pipleline = ResampleRegressor(
        base_estimator=lr_zscore,
        low_thresh=0.2,
        high_thresh=0.8,
        upsample_ratio=0.5,
    )
    ...
    grid_search_zscore = GridSearchCV(
        resampled_pipleline,
        param_grid={
            "base_estimator__elasticnet__alpha": parameters.alphas,
            "base_estimator__elasticnet__l1_ratio": parameters.l1_r,
        },
    ...
    non_zero_coefs_df.to_csv(
        combination_path / "common_proteins_df_resample.csv", index=False
    )
    ...
    if cfg.save:
        pd.DataFrame(cv_results_zscore).to_csv(
            combination_path / "cv_results_zscore_resample.csv"
        )
        scores_matrix.to_csv(combination_path / "scores_matrix_resample.csv")
        pd.DataFrame(best_coef_df).to_csv(
            combination_path / "best_coef_df_resample.csv"
        )
        top_coef_df.to_csv(combination_path / "top_coef_df_resample.csv")
        data_testing_with_prediction.to_csv(
            combination_path / "data_testing_with_prediction_May8th_resample.csv",
            index=False,
        )
...
def hyperparameters_tuning_for_gradientboosting(
    path: Path = Path("hyperparameters_tuning"),
    parameters: Params = Params(),
    cfg: Config = Config(),
):
    ...
    gb_pipeline = make_pipeline(WeightedZScoreNormalizer(), GradientBoostingRegressor())

    resampled_pipleline = ResampleRegressor(
        base_estimator=gb_pipeline,
        low_thresh=0.2,
        high_thresh=0.8,
        upsample_ratio=0.5,
    )
    ...
    grid_search_zscore = GridSearchCV(
        resampled_pipleline,
        param_grid={
            "base_estimator__gradientboostingregressor__alpha": parameters.alphas,
            "base_estimator__gradientboostingregressor__l1_ratio": parameters.l1_r,
        },
        cv=cv_in,  # Internal CV
        n_jobs=get_jobs(),
        scoring=parameters.scoring,
